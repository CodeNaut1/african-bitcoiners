...
import { CARDS, LINKS, type OutreachCard } from '@/components/CommunityOutreachPage/data'
...
function OutreachCardItem({ card }: { card: OutreachCard }) {
  return (
...
export function CommunityOutreachPage() {
  return (
      ...
      <section className="px-4 pt-[30px] sm:px-6 md:pt-[50px]">
        <div className="mx-auto max-w-[800px] text-center">
          ...
          <p className="mt-4 text-lg tracking-[-0.4px]" style={{ color: HEADING }}>
            We meet people where they are — in towns, campuses, workplaces, and online communities — to share practical
            Bitcoin education and help Africans take their next step with confidence.
          </p>
          <p className="mt-4 text-base tracking-[-0.2px]" style={{ color: BODY }}>
            Explore the initiatives below to get involved, partner with us, or bring Bitcoin outreach to your community.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 pb-6 pt-6 sm:px-6">
        <div className="mx-auto max-w-[1000px]">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {CARDS.map((card) => (
              <OutreachCardItem key={card.title} card={card} />
            ))}
          </div>
        </div>
      </section>

      <section className="px-4 pb-[30px] sm:px-6 md:pb-[50px]">
        <div className="mx-auto max-w-[800px] rounded border border-black/10 bg-white px-6 py-8 text-center">
          <h2 className="font-heading text-2xl font-bold tracking-[-0.4px]" style={{ color: HEADING }}>
            Stay connected
          </h2>
          <p className="mt-3 text-base leading-7 tracking-[-0.2px]" style={{ color: BODY }}>
            Join the wider African Bitcoiners community or subscribe to our newsletter for updates on meetups, outreach
            events, and new resources.
          </p>
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              href={LINKS.community}
              className="inline-block rounded px-10 py-3 text-base font-semibold text-white transition-colors hover:bg-[#253343]"
            ...
            <Link
              ...
              className="inline-block rounded border border-black/15 px-10 py-3 text-base font-semibold transition-colors hover:bg-black/5"
              style={{ color: HEADING }}
            >
              Read the Newsletter
            </Link>
