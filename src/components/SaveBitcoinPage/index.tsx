...
import { CARDS, LINKS, type SaveCard } from '@/components/SaveBitcoinPage/data'
...
function SaveCardItem({ card }: { card: SaveCard }) {
  return (
...
export function SaveBitcoinPage() {
  return (
      ...
      <section className="px-4 pb-[30px] pt-6 sm:px-6 md:pb-[50px]">
        <div className="mx-auto max-w-[1000px]">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {CARDS.map((card) => (
              <SaveCardItem key={card.title} card={card} />
            ))}
          </div>
        </div>
      </section>
