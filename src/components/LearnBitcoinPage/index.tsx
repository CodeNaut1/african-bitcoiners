...
import { CARDS, LINKS, type LearnCard } from '@/components/LearnBitcoinPage/data'
...
function LearnCardItem({ card }: { card: LearnCard }) {
  return (
      ...
      <section className="px-4 pt-[30px] sm:px-6 md:pt-[50px]">
        <div className="mx-auto max-w-[800px] text-center">
          ...
          <p className="mt-4 text-lg tracking-[-0.4px]" style={{ color: HEADING }}>
            Kickstart your Bitcoin Learning Journey Here!
          </p>
        </div>
      </section>
      ...
      <section className="px-4 pb-[30px] pt-6 sm:px-6 md:pb-[50px]">
        <div className="mx-auto max-w-[1000px]">
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {CARDS.map((card) => (
              <LearnCardItem key={card.title} card={card} />
            ))}
          </div>
        </div>
      </section>
