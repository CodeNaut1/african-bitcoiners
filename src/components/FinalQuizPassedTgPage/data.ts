...
export const LINKS = {
  getCertificate: '/get-certificate-tg',
}
...
export function certificateHref(uniqueId?: string | null) {
  if (!uniqueId) return LINKS.getCertificate
  return `${LINKS.getCertificate}?uniqueId=${encodeURIComponent(uniqueId)}`
}
