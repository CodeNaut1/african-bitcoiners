...
function XIcon() {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 64 64" fill="none" aria-hidden>
...
function TeamCard({
  ...
  const widthClass =
    imageScale === '84'
      ? 'mx-auto block w-[92%]'
      : imageScale === '90'
        ? 'mx-auto block w-[88%] md:w-[95%]'
        : 'block w-full'

  return (
    <article className={indexInRow > 0 ? 'mt-[30px] md:mt-0' : ''}>
      <div
        className={`overflow-hidden rounded-tl-[24px] rounded-br-[24px] ${framePadTop ? 'pt-6' : ''} ${imageScale ? 'text-center' : ''}`}
      ...
      >
        <Image
          ...
          width={imageW}
          height={imageH}
          className={`h-auto ${widthClass} ${imageCorner ? 'rounded-br-[24px] rounded-bl-[12px]' : ''}`}
          sizes="(max-width: 768px) 78vw, 280px"
        />
      </div>
      <p className="mt-4 font-sans text-lg font-bold leading-snug" style={{ color: TEXT }}>
        {name}
      </p>
      <p className="mt-1.5 font-sans text-lg leading-[26px]" style={{ color: TEXT }}>
        {role}
      </p>
      <a
        href={xUrl}
        ...
        className={`inline-flex transition-opacity hover:opacity-80 ${socialPadTop ? 'mt-6' : 'mt-4'}`}
      ...
      <div className="mx-auto max-w-[1200px]">
        ...
        <div className="mx-auto max-w-[820px]">
          {ROWS.map((row, rowIndex) => (
            <div
              key={rowIndex}
              className={`grid grid-cols-1 px-[25px] md:grid-cols-[30%_5%_30%_5%_30%] md:px-6 ${rowIndex === 0 ? 'pt-5' : 'pt-10'}`}
            >
              {row.length === 1 ? (
                <>
