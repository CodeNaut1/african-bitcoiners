...
export default function ImportNewsletterView() {
  ...
  const [success, setSuccess] = useState<{ postId: string | number; slug: string; status: string } | null>(
    null,
  )
  ...
  const submitPost = async (status: 'published' | 'draft') => {
    ...
    try {
      ...
      setSuccess({
        postId: data.postId!,
        slug: data.slug!,
        status: status === 'published' ? 'Published' : 'Saved as draft',
      })
    } catch (err) {
      ...
      <div style={{ maxWidth: '1100px', paddingTop: 'var(--base)', paddingBottom: 'calc(var(--base) * 2)' }}>
        ...
        {success && (
          <div style={{ marginBottom: '1rem' }}>
            <Banner type="success">
              {success.status}!{' '}
              <a href={`/admin/collections/posts/${success.postId}`} style={{ color: 'inherit' }}>
                View in admin
              </a>
              {' · '}
              <a href={`/bitcoin-newsletter/${success.slug}`} target="_blank" rel="noreferrer" style={{ color: 'inherit' }}>
                View on site
              </a>
            </Banner>
          </div>
